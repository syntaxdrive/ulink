    ...
    try {
      ...
      if (accessToken && refreshToken) {
        ...
        if (data?.session?.user) {
          ...
          // Sync with NestJS backend if reachable
          try {
            const { apiClient } = await import('../api/client');
            const nestAuth = await apiClient.post('/auth/google-profile', {
            ...
            if (nestAuth.data?.access_token) {
              await get().setToken(nestAuth.data.access_token, nestAuth.data.user?.id || data.session.user.id);
            }
          } catch (e) {
            console.warn('NestJS auth sync skipped (running Supabase fallback session):', e);
          }
          return true;
        }
      } else if (code) {
