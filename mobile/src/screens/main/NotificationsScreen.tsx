...
export default function NotificationsScreen() {
  ...
  useEffect(() => {
    ...
    // 1. Primary: Socket.io Realtime Notification Gateway
    getNotifSocket()
      .then((socket) => {
        notifSocket = socket;
      ...
      .catch((err) => {
        console.warn('Notification socket connection failed, using Supabase fallback:', err);
      });
    ...
    return () => {
      if (notifSocket) {
        ...
        notifSocket.off('connectionRequest');
        notifSocket.off('connectionAccepted');
      }
      if (channel) {
  ...
  const handleNotificationPress = async (item: Notification) => {
    ...
    // 1. Community Routes
    if (url.includes('/communities/') || type.includes('community')) {
      const commSlug = url.includes('/communities/') ? url.split('/communities/')[1]?.split('/')[0] : null;
      if (commSlug) {
    ...
    // 2. Podcast Routes
    if (url.includes('/podcasts/') || type.includes('podcast')) {
      const podId = url.includes('/podcasts/') ? url.split('/podcasts/')[1]?.split('/')[0] : null;
      if (podId) {
    ...
    // 3. Direct Message Routes
    if (type === 'message' || url.includes('/messages')) {
      navigation.navigate('Messages');
      return;
    }

    // 4. Follows & Connections -> User Profile
    if (type === 'follow' || type.includes('connection')) {
      if (url.includes('/u/') || url.includes('/profile/')) {
        const targetHandleOrId = url.includes('/u/') ? url.split('/u/')[1]?.split('/')[0] : url.split('/profile/')[1]?.split('/')[0];
        if (targetHandleOrId) {
          navigation.navigate('Profile', { userId: targetHandleOrId });
          return;
        }
      }
    ...
    // 5. Posts / Likes / Comments / Reposts -> Feed
    if (type === 'like' || type === 'comment' || type === 'repost' || url.includes('/post/') || url.includes('/posts/')) {
      navigation.navigate('Feed');
      return;
    }

    // 6. Study Room Invites
    if (type === 'study_invite' || url.includes('/study')) {
