    ...
    <SafeAreaView style={[styles.container, { backgroundColor: isDark ? '#000000' : '#FFFFFF' }]}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.iconBtn}>
          <Settings size={24} color={isDark ? '#FFFFFF' : '#111827'} />
        </TouchableOpacity>
      </View>

      {/* Room Info */}
      <View style={[styles.roomInfo, { borderBottomColor: isDark ? '#1F1F23' : '#E5E7EB' }]}>
        <Text style={[styles.roomTitle, { color: isDark ? '#FFFFFF' : '#111827' }]}>{room.title}</Text>
        <Text style={[styles.roomTopic, { color: isDark ? '#9CA3AF' : '#6B7280' }]}>{room.topic || 'General Discussion'}</Text>
        <View style={styles.statsRow}>
          <Radio size={16} color="#10B981" />
          <Text style={[styles.statsText, { color: '#10B981' }]}>{allParticipants.length} Listening</Text>
        </View>
      </View>
      ...
      {/* Bottom Controls */}
      <View style={[styles.controlsContainer, { backgroundColor: isDark ? '#121214' : '#FFFFFF', borderTopColor: isDark ? '#1F1F23' : '#E5E7EB' }]}>
        <TouchableOpacity style={[styles.leaveBtn, { backgroundColor: isDark ? '#450A0A' : '#FEF2F2' }]} onPress={handleLeaveRoom}>
          <LogOut size={20} color="#EF4444" />
          <Text style={[styles.leaveBtnText, { color: '#EF4444' }]}>Leave</Text>
        </TouchableOpacity>

        <TouchableOpacity style={[styles.circleBtn, { backgroundColor: isDark ? '#1E1E24' : '#F3F4F6' }]} onPress={() => setHandRaised(!handRaised)}>
          <Hand size={24} color={handRaised ? '#10B981' : (isDark ? '#FFFFFF' : '#111827')} />
        </TouchableOpacity>
        ...
        >
          {isMuted ? (
            ...
            <Mic size={24} color={isDark ? '#FFFFFF' : '#111827'} />
          )}
        </TouchableOpacity>
      </View>
    </SafeAreaView>
