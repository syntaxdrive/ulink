...
export default function CommunityDetailScreen() {
  ...
  const communityCommentInputRef = useRef<TextInput>(null);
  ...
  const fetchCommunity = useCallback(async () => {
    try {
      ...
      if (uid) {
        ...
        const userIsMember = (!!member && (member.status === 'active' || !member.status)) || (data.creator_id === uid);
  ...
  /* ── 9b. Repost & Quote Modals ── */
  const handleOpenRepostMenu = (post: Post) => {
  ...
  const handleInstantRepost = async () => {
    ...
    const uid = currentUserId || useAuthStore.getState().userId;
    if (!uid) {
    ...
    setRepostMenuVisible(false);
    try {
      const success = await FeedService.repost(repostTargetPost.id, uid);
      if (success) {
  ...
  const handleSubmitQuoteRepost = async () => {
    ...
    setReposting(true);
    try {
      const success = await FeedService.repost(repostTargetPost.id, uid, quoteComment);
      if (success) {
        setQuoteModalVisible(false);
        setQuoteComment('');
  ...
  const toggleSavePost = async (postOrId: string | any) => {
    ...
    if (uid) {
      try {
        await FeedService.toggleBookmark(fullPost || postId, uid, isCurrentlySaved);
      } catch (err) {
        console.warn('Error toggling bookmark in community:', err);
      }
    }
  };
  ...
  const handleLike = async (postId: string, isLiked: boolean) => {
      ...
      prev.map((p) =>
        p.id === postId
          ? {
              ...
              likes_count: Math.max(0, isLiked ? p.likes_count - 1 : p.likes_count + 1),
            }
          : p
      )
    );
    ...
    try {
      await FeedService.toggleLike(postId, uid, isLiked);
    } catch (err) {
      console.warn('Like toggle error in community:', err);
      // Re-sync posts if failed
      if (community?.id) fetchPosts(community.id, uid);
    }
  };
  ...
  const openComments = async (postId: string) => {
    ...
    try {
      const uid = currentUserId || useAuthStore.getState().userId;
      const fetched = await FeedService.getComments(postId, uid);
  ...
  const handleStartReply = (comment: any) => {
    ...
    setTimeout(() => {
      communityCommentInputRef.current?.focus();
    }, 100);
  };
  ...
  const handleToggleLikeComment = async (comment: any) => {
    ...
    try {
      await FeedService.toggleLikeComment(comment.id, uid, isCurrentlyLiked);
    } catch (e) {
      console.warn('Error liking community comment:', e);
    }
  };

  const submitComment = async () => {
    const uid = currentUserId || useAuthStore.getState().userId;
    if (!commentText.trim() || !activePostId || !uid) return;
    ...
    setReplyingToComment(null);
    try {
      const newC = await FeedService.addComment(activePostId, uid, text, parentId, replyToUsername);
      if (newC) {
    ...
    Alert.alert('Delete Comment', 'Are you sure you want to delete your comment?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          try {
            await FeedService.deleteComment(commentId, activePostId, uid);
          } catch {
  ...
  const renderPost = ({ item }: { item: Post }) => {
    const uid = currentUserId || useAuthStore.getState().userId;
    const isPostAuthor =
      (item.author_id && item.author_id === uid) ||
      (item.author?.id && item.author.id === uid);
    const canDelete = isPostAuthor || isAdmin;
    const isSaved = savedPosts.has(item.id);

    return (
      <View style={[styles.postCard, { backgroundColor: isDark ? '#000000' : '#FFFFFF' }]}>
        {item.is_repost && (
          <View style={styles.repostBanner}>
            <Repeat2 size={12} color="#10B981" />
            <Text style={[styles.repostText, { color: isDark ? '#9CA3AF' : '#6B7280' }]}> Reposted</Text>
          </View>
        )}
        <View style={styles.postHeader}>
          <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
            {item.author?.avatar_url ? (
              ...
              <View style={[styles.authorAvatar, styles.avatarPlaceholder, { backgroundColor: '#10B981' }]}>
                <Text style={styles.avatarInitial}>{(item.author?.name || 'U')[0].toUpperCase()}</Text>
              </View>
            )}
            <View style={styles.authorInfo}>
              <View style={styles.nameRow}>
                <Text style={[styles.authorName, { color: isDark ? '#FFFFFF' : '#111827' }]}>{item.author?.name || 'Student'}</Text>
                {item.author?.is_verified && (
                  <VerifiedBadge size={14} />
                )}
                {item.created_at && (
                  <Text style={[styles.authorTimestamp, { color: isDark ? '#9CA3AF' : '#6B7280' }]}>
                    {' · '}{formatRelativeTime(item.created_at)}
                  </Text>
                )}
              </View>
              <Text style={[styles.authorMeta, { color: isDark ? '#71717A' : '#9CA3AF' }]}>
          ...
          {canDelete ? (
            ...
            >
              <Flag size={14} color={isDark ? '#9CA3AF' : '#6B7280'} />
            </TouchableOpacity>
          )}
        </View>
        ...
        {item.content ? (
          <>
            <FormattedText content={item.content} style={[styles.postContent, { color: isDark ? '#E4E4E7' : '#1F2937' }]} />
        ...
        <View style={styles.postActions}>
          <TouchableOpacity style={styles.actionBtn} onPress={() => handleLike(item.id, !!item.user_has_liked)}>
            <Heart
              size={18}
              color={item.user_has_liked ? '#EF4444' : (isDark ? '#9CA3AF' : '#6B7280')}
              fill={item.user_has_liked ? '#EF4444' : 'transparent'}
            />
            <Text style={[styles.actionCount, { color: item.user_has_liked ? '#EF4444' : (isDark ? '#9CA3AF' : '#6B7280') }]}>
              {item.likes_count || 0}
            </Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity style={styles.actionBtn} onPress={() => openComments(item.id)}>
            ...
            <Text style={[styles.actionCount, { color: isDark ? '#9CA3AF' : '#6B7280' }]}>{item.comments_count || 0}</Text>
          </TouchableOpacity>

          <TouchableOpacity style={styles.actionBtn} onPress={() => handleOpenRepostMenu(item)}>
            <Repeat2 size={18} color={item.is_repost ? '#10B981' : (isDark ? '#9CA3AF' : '#6B7280')} />
          </TouchableOpacity>

          <TouchableOpacity style={styles.actionBtn} onPress={() => toggleSavePost(item)}>
            <Bookmark
              size={18}
              color={isSaved ? '#10B981' : (isDark ? '#9CA3AF' : '#6B7280')}
              fill={isSaved ? '#10B981' : 'transparent'}
            />
          </TouchableOpacity>
          ...
          >
            <Share2 size={17} color={isDark ? '#9CA3AF' : '#6B7280'} />
          </TouchableOpacity>
        </View>
      </View>
    );
    ...
    return (
      <View style={styles.communityHeader}>
        ...
        <View style={styles.communityInfoRow}>
          ...
          <View style={styles.headerBtnsRow}>
            ...
            <TouchableOpacity style={styles.membersBtn} onPress={openMembersModal}>
              <Users size={14} color={isDark ? '#FFFFFF' : '#111827'} />
              <Text style={[styles.membersBtnText, { color: isDark ? '#FFFFFF' : '#111827' }]}>Members</Text>
            </TouchableOpacity>

            {/* Community Settings */}
            <TouchableOpacity style={styles.membersBtn} onPress={() => setIsSettingsOpen(true)}>
              <Settings size={14} color={isDark ? '#FFFFFF' : '#111827'} />
              <Text style={[styles.membersBtnText, { color: isDark ? '#FFFFFF' : '#111827' }]}>Settings</Text>
            </TouchableOpacity>
        ...
        <View style={styles.communityMeta}>
          <Text style={[styles.communityName, { color: isDark ? '#FFFFFF' : '#111827' }]}>{community.name}</Text>
          <View style={styles.communityStats}>
            <Users size={13} color={isDark ? '#9CA3AF' : '#6B7280'} />
            <Text style={[styles.communityStatText, { color: isDark ? '#9CA3AF' : '#6B7280' }]}>{community.members_count || 0} members</Text>
            {community.privacy === 'public' ? (
              <>
                <Globe size={13} color="#10B981" />
                <Text style={[styles.communityStatText, { color: '#10B981' }]}>Public</Text>
              </>
            ) : (
              <>
                <Lock size={13} color="#EF4444" />
                <Text style={[styles.communityStatText, { color: '#EF4444' }]}>Private</Text>
              </>
            )}
          </View>
          {community.description ? (
            <Text style={[styles.communityDescription, { color: isDark ? '#9CA3AF' : '#4B5563' }]}>{community.description}</Text>
          ) : null}
        </View>
        ...
        {isMember ? (
          <View style={[styles.inCommunityComposer, { backgroundColor: isDark ? '#18181B' : '#FFFFFF', borderColor: isDark ? '#27272A' : '#E5E7EB' }]}>
            <TextInput
              style={[
                styles.composerInput,
                { color: isDark ? '#FFFFFF' : '#111827', minHeight: 48 },
              ]}
            ...
            {attachedVideo && (
              <View style={styles.videoBadgeRow}>
                <VideoIcon size={14} color="#10B981" />
                <Text style={[styles.videoBadgeText, { color: '#10B981' }]} numberOfLines={1}>
            ...
            <View style={[styles.composerActions, { borderTopColor: isDark ? '#27272A' : '#E5E7EB' }]}>
              <View style={styles.composerMediaIcons}>
                ...
                >
                  <VideoIcon size={18} color="#10B981" />
                </TouchableOpacity>
              </View>
              <TouchableOpacity
                style={[
          ...
          <TouchableOpacity
            style={[
              styles.joinToPostCard,
              {
                backgroundColor: isDark ? '#18181B' : '#ECFDF5',
                borderColor: isDark ? '#27272A' : '#A7F3D0',
              },
            ]}
            onPress={handleJoinLeave}
            activeOpacity={0.85}
          >
            <View style={{ flex: 1 }}>
              <Text style={[styles.joinToPostTitle, { color: isDark ? '#FFFFFF' : '#065F46' }]}>
                Join this community to post
              </Text>
              <Text style={[styles.joinToPostSubtitle, { color: isDark ? '#9CA3AF' : '#047857' }]}>
                Connect with members and share your ideas, notes, or questions.
              </Text>
            </View>
            <View style={styles.joinToPostBtnBadge}>
              <Text style={styles.joinToPostBtnText}>Join</Text>
            </View>
          </TouchableOpacity>
        )}

        <View style={[styles.postsLabel, { borderTopColor: isDark ? '#27272A' : '#E5E7EB' }]}>
          <Text style={[styles.postsSectionTitle, { color: isDark ? '#A1A1AA' : '#6B7280' }]}>Community Discussions</Text>
        </View>
      </View>
    );
  };
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: isDark ? '#000000' : '#FFFFFF' }]}>
      ...
      <View style={[styles.header, { borderBottomColor: isDark ? '#1F1F23' : '#E5E7EB' }]}>
        ...
        <Text style={[styles.headerTitle, { color: isDark ? '#FFFFFF' : '#111827' }]} numberOfLines={1}>
        ...
        <View style={styles.topRightActions}>
          ...
          <TouchableOpacity style={styles.iconBtn} onPress={() => setIsSettingsOpen(true)}>
            <Settings size={20} color={isDark ? '#FFFFFF' : '#111827'} />
          </TouchableOpacity>
        </View>
      </View>
        ...
        <SafeAreaView style={[styles.modalSafeArea, { backgroundColor: isDark ? '#000000' : '#FFFFFF' }]}>
          ...
          >
            <View style={[styles.modalHeader, { borderBottomColor: isDark ? '#1F1F23' : '#E5E7EB' }]}>
              ...
              <Text style={[styles.modalTitle, { color: isDark ? '#FFFFFF' : '#111827' }]}>Community Settings</Text>
              <TouchableOpacity
            ...
            <ScrollView contentContainerStyle={styles.settingsFormScroll} showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.formSection}>
                <Text style={[styles.formLabel, { color: isDark ? '#9CA3AF' : '#6B7280' }]}>Community Cover Banner</Text>
                <TouchableOpacity
                  style={[styles.coverUploadBox, { backgroundColor: isDark ? '#18181B' : '#F9FAFB', borderColor: isDark ? '#27272A' : '#E5E7EB' }]}
                  onPress={async () => {
                ...
                >
                  {editCoverUri || community?.cover_url ? (
                    ...
                    <View style={styles.uploadPlaceholder}>
                      ...
                      <Text style={[styles.uploadText, { color: isDark ? '#9CA3AF' : '#6B7280' }]}>Upload Cover Banner</Text>
                    </View>
                  )}
                </TouchableOpacity>
              </View>
              ...
              {/* Logo / Icon DP Upload */}
              <View style={styles.formSection}>
                <Text style={[styles.formLabel, { color: isDark ? '#9CA3AF' : '#6B7280' }]}>Community Display Picture (DP) / Icon</Text>
                <TouchableOpacity
                  style={[styles.logoUploadBox, { backgroundColor: isDark ? '#18181B' : '#F9FAFB', borderColor: isDark ? '#27272A' : '#E5E7EB' }]}
                  onPress={async () => {
                    const media = await uploadService.pickImages(1);
                ...
                >
                  {editIconUri || community?.icon_url ? (
                    ...
                    <View style={styles.uploadPlaceholder}>
                      <Camera size={24} color="#10B981" />
                      <Text style={[styles.uploadText, { color: isDark ? '#9CA3AF' : '#6B7280' }]}>Upload Community DP</Text>
                    </View>
                  )}
                </TouchableOpacity>
              </View>
              ...
              {/* Name */}
              <View style={styles.formSection}>
                <Text style={[styles.formLabel, { color: isDark ? '#9CA3AF' : '#6B7280' }]}>Community Name</Text>
                <TextInput
                  style={[styles.formInput, { backgroundColor: isDark ? '#18181B' : '#F9FAFB', borderColor: isDark ? '#27272A' : '#E5E7EB', color: isDark ? '#FFFFFF' : '#111827' }]}
                  value={editName}
              ...
              <View style={styles.formSection}>
                <Text style={[styles.formLabel, { color: isDark ? '#9CA3AF' : '#6B7280' }]}>Privacy</Text>
                <View style={styles.privacyRow}>
                  <TouchableOpacity
                    style={[styles.privacyBtn, { backgroundColor: isDark ? '#18181B' : '#FFFFFF', borderColor: isDark ? '#27272A' : '#E5E7EB' }, editPrivacy === 'public' && styles.privacyBtnActive]}
                    onPress={() => setEditPrivacy('public')}
                  >
                    <Globe size={18} color={editPrivacy === 'public' ? '#10B981' : (isDark ? '#9CA3AF' : '#6B7280')} />
                    <View style={{ marginLeft: 10 }}>
                      <Text style={[styles.privacyTitle, { color: isDark ? '#FFFFFF' : '#111827' }, editPrivacy === 'public' && styles.privacyTitleActive]}>
                        Public
                      </Text>
                      <Text style={[styles.privacySub, { color: isDark ? '#9CA3AF' : '#6B7280' }]}>Open to all campus students</Text>
                    </View>
                  </TouchableOpacity>

                  <TouchableOpacity
                    style={[styles.privacyBtn, { backgroundColor: isDark ? '#18181B' : '#FFFFFF', borderColor: isDark ? '#27272A' : '#E5E7EB' }, editPrivacy === 'private' && styles.privacyBtnActive]}
                    onPress={() => setEditPrivacy('private')}
                  >
                    <Lock size={18} color={editPrivacy === 'private' ? '#10B981' : (isDark ? '#9CA3AF' : '#6B7280')} />
                    <View style={{ marginLeft: 10 }}>
                      <Text style={[styles.privacyTitle, { color: isDark ? '#FFFFFF' : '#111827' }, editPrivacy === 'private' && styles.privacyTitleActive]}>
                        Private
                      </Text>
                      <Text style={[styles.privacySub, { color: isDark ? '#9CA3AF' : '#6B7280' }]}>Invite & approval required</Text>
                    </View>
                  </TouchableOpacity>
                </View>
              </View>
              ...
              {/* Posting Permissions */}
              <View style={styles.formSection}>
                <Text style={[styles.formLabel, { color: isDark ? '#9CA3AF' : '#6B7280' }]}>Posting Permissions</Text>
                <View style={styles.privacyRow}>
                  <TouchableOpacity
                    style={[styles.privacyBtn, { backgroundColor: isDark ? '#18181B' : '#FFFFFF', borderColor: isDark ? '#27272A' : '#E5E7EB' }, editPostingPermission === 'all' && styles.privacyBtnActive]}
                    onPress={() => setEditPostingPermission('all')}
                  >
                    <Users size={18} color={editPostingPermission === 'all' ? '#10B981' : (isDark ? '#9CA3AF' : '#6B7280')} />
                    <View style={{ marginLeft: 10 }}>
                      <Text style={[styles.privacyTitle, { color: isDark ? '#FFFFFF' : '#111827' }, editPostingPermission === 'all' && styles.privacyTitleActive]}>
                        All Members
                      </Text>
                      <Text style={[styles.privacySub, { color: isDark ? '#9CA3AF' : '#6B7280' }]}>Any joined member can post</Text>
                    </View>
                  </TouchableOpacity>

                  <TouchableOpacity
                    style={[styles.privacyBtn, { backgroundColor: isDark ? '#18181B' : '#FFFFFF', borderColor: isDark ? '#27272A' : '#E5E7EB' }, editPostingPermission === 'admins_only' && styles.privacyBtnActive]}
                    onPress={() => setEditPostingPermission('admins_only')}
                  >
                    <Crown size={18} color={editPostingPermission === 'admins_only' ? '#10B981' : (isDark ? '#9CA3AF' : '#6B7280')} />
                    <View style={{ marginLeft: 10 }}>
                      <Text style={[styles.privacyTitle, { color: isDark ? '#FFFFFF' : '#111827' }, editPostingPermission === 'admins_only' && styles.privacyTitleActive]}>
                        Admins Only
                      </Text>
                      <Text style={[styles.privacySub, { color: isDark ? '#9CA3AF' : '#6B7280' }]}>Only group leaders & admins can post</Text>
                    </View>
                  </TouchableOpacity>
                </View>
              </View>
              ...
              {/* Description */}
              <View style={styles.formSection}>
                <Text style={[styles.formLabel, { color: isDark ? '#9CA3AF' : '#6B7280' }]}>Description</Text>
                <TextInput
                  style={[styles.formInput, styles.formTextArea, { backgroundColor: isDark ? '#18181B' : '#F9FAFB', borderColor: isDark ? '#27272A' : '#E5E7EB', color: isDark ? '#FFFFFF' : '#111827' }]}
              ...
              {/* Rules & Guidelines */}
              <View style={styles.formSection}>
                <Text style={[styles.formLabel, { color: isDark ? '#9CA3AF' : '#6B7280' }]}>Community Rules & Guidelines</Text>
                <TextInput
                  style={[styles.formInput, styles.formTextArea, { backgroundColor: isDark ? '#18181B' : '#F9FAFB', borderColor: isDark ? '#27272A' : '#E5E7EB', color: isDark ? '#FFFFFF' : '#111827' }]}
                  value={editRules}
              ...
              <View style={[styles.dangerSection, { borderTopColor: isDark ? '#27272A' : '#E5E7EB' }]}>
      ...
      >
        <SafeAreaView style={[styles.modalSafeArea, { backgroundColor: isDark ? '#000000' : '#FFFFFF' }]}>
          <View style={[styles.modalHeader, { borderBottomColor: isDark ? '#1F1F23' : '#E5E7EB' }]}>
            ...
            <Text style={[styles.modalTitle, { color: isDark ? '#FFFFFF' : '#111827' }]}>Manage Members ({members.length})</Text>
            <View style={{ width: 40 }} />
          </View>

          {/* Add Member Search Bar */}
          {isAdmin && (
            <View style={[styles.addMemberSearchRow, { borderBottomColor: isDark ? '#1F1F23' : '#E5E7EB' }]}>
              <TextInput
                style={[styles.addMemberInput, { backgroundColor: isDark ? '#18181B' : '#F9FAFB', borderColor: isDark ? '#27272A' : '#E5E7EB', color: isDark ? '#FFFFFF' : '#111827' }]}
          ...
          {searchedUsers.length > 0 && (
            <View style={[styles.searchResultsBox, { backgroundColor: isDark ? '#18181B' : '#F9FAFB', borderBottomColor: isDark ? '#1F1F23' : '#E5E7EB' }]}>
              <Text style={[styles.searchHeader, { color: isDark ? '#9CA3AF' : '#6B7280' }]}>Search Results:</Text>
              {searchedUsers.map((u) => (
                <View key={u.id} style={styles.searchResultItem}>
                  <Text style={[styles.searchUserName, { color: isDark ? '#FFFFFF' : '#111827' }]}>{u.name || u.username}</Text>
                  <TouchableOpacity
          ...
          {loadingMembers ? (
            ...
            <ScrollView contentContainerStyle={styles.membersListScroll}>
              {members.map((m) => {
                ...
                return (
                  <View key={m.id} style={[styles.memberCard, { backgroundColor: isDark ? '#18181B' : '#FFFFFF', borderColor: isDark ? '#27272A' : '#F3F4F6' }]}>
                    {m.profile?.avatar_url ? (
                      <Image source={{ uri: m.profile.avatar_url }} style={styles.memberAvatar} />
                    ) : (
                      <View style={[styles.memberAvatar, styles.avatarPlaceholder, { backgroundColor: '#10B981' }]}>
                    ...
                    <View style={styles.memberMeta}>
                      <View style={styles.nameRow}>
                        <Text style={[styles.memberName, { color: isDark ? '#FFFFFF' : '#111827' }]}>{m.profile?.name || 'Student'}</Text>
                        {isUserAdmin && (
                      ...
                      <Text style={[styles.memberHandle, { color: isDark ? '#71717A' : '#9CA3AF' }]}>@{m.profile?.username || 'user'}</Text>
                    </View>
                    ...
                    {isAdmin && !isSelf && (
                      <View style={styles.memberActionsRow}>
                        <TouchableOpacity
                          style={[styles.promoteBtn, { backgroundColor: isDark ? '#27272A' : '#F3F4F6' }]}
                          onPress={() => handleToggleAdminRole(m)}
                        >
                          <Text style={[styles.promoteBtnText, { color: isDark ? '#FFFFFF' : '#111827' }]}>
      ...
      <Modal visible={commentsVisible} animationType="slide" transparent>
        ...
        >
          <View style={[styles.commentsSheet, { backgroundColor: isDark ? '#121214' : '#FFFFFF' }]}>
            <View style={[styles.commentsHeader, { borderBottomColor: isDark ? '#1F1F23' : '#E5E7EB' }]}>
              <Text style={[styles.commentsTitle, { color: isDark ? '#FFFFFF' : '#111827' }]}>Comments ({comments.length})</Text>
            ...
            {commentsLoading ? (
              ...
              <ScrollView style={styles.commentsList} contentContainerStyle={{ paddingBottom: 20 }}>
                {comments.length === 0 ? (
                  <Text style={[styles.noCommentsText, { color: isDark ? '#9CA3AF' : '#6B7280' }]}>No comments yet. Start the conversation!</Text>
                ) : (
                  comments.map((comment) => {
                    const uid = currentUserId || useAuthStore.getState().userId;
                    const isMyComment = comment.author?.id === uid || (comment as any).author_id === uid;
                    const isLiked = likedCommentIds.has(comment.id) || !!comment.user_has_liked;
                    ...
                    return (
                      ...
                      >
                        <View style={[styles.commentBubble, { backgroundColor: isDark ? '#18181B' : '#F3F4F6' }]}>
                          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 2 }}>
                            <View style={{ flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap' }}>
                              <Text style={[styles.commentAuthor, { color: isDark ? '#FFFFFF' : '#111827', fontWeight: '700' }]}>
                                {comment.author?.name || comment.author?.username || 'Student'}
                              </Text>
                          ...
                          <Text style={[styles.commentContent, { color: isDark ? '#E4E4E7' : '#1F2937' }]}>{comment.content}</Text>

                          {/* Comment Footer Actions: Reply & Like */}
                          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 6, paddingTop: 4, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: isDark ? '#27272A' : '#E5E7EB' }}>
                            <TouchableOpacity
                            ...
                            <TouchableOpacity
                              ...
                              onPress={() => handleToggleLikeComment(comment)}
                              hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}
                            >
                              <Heart
                                size={14}
                              ...
                              {(comment.likes_count || 0) > 0 && (
                                <Text style={{ fontSize: 11, color: isLiked ? '#EF4444' : (isDark ? '#71717A' : '#9CA3AF'), marginLeft: 4, fontWeight: '600' }}>
                                  {comment.likes_count}
                                </Text>
                              )}
                            </TouchableOpacity>
            ...
            {replyingToComment && (
              ...
              >
                <Text style={{ fontSize: 12, color: isDark ? '#93C5FD' : '#1D4ED8' }}>
                ...
                <TouchableOpacity onPress={handleCancelReply} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
                  <X size={14} color={isDark ? '#93C5FD' : '#1D4ED8'} />
                </TouchableOpacity>
              </View>
            )}

            <View style={[styles.commentInputRow, { backgroundColor: isDark ? '#121214' : '#FFFFFF', borderTopColor: isDark ? '#1F1F23' : '#E5E7EB' }]}>
              <TextInput
                ref={communityCommentInputRef}
                style={[styles.commentInput, { backgroundColor: isDark ? '#18181B' : '#F3F4F6', color: isDark ? '#FFFFFF' : '#111827' }]}
                placeholder={replyingToComment ? `Reply to @${replyingToComment.author?.username || 'student'}...` : "Write a comment..."}
                placeholderTextColor={isDark ? '#71717A' : '#9CA3AF'}
                value={commentText}
