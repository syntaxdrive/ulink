  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const res = await apiClient.get(`/profiles/search?q=${encodeURIComponent(trimmed)}`);
        if (res.data && Array.isArray(res.data) && res.data.length > 0) {
          const match = res.data.find((u: any) => u.email?.toLowerCase() === trimmed);
          if (match) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isLogin) {
        ...
        // 2. Fallback: Supabase Sign In (if NestJS was unreachable)
        try {
  ...
  return (
    <View style={styles.container}>
      <SafeAreaView style={styles.safeArea}>
        ...
        >
          ...
          >
            ...
            <View style={styles.formContainer}>
              ...
              {/* Legal & UGC Compliance Notice on Sign Up */}
              {!isLogin && (
        ...
        {/* ── In-App Legal & Privacy Modal (Play Store Compliance) ── */}
        <LegalModal
