...
export const uploadService = {
  ...
  async uploadFile(media: PickedMedia, folder = 'posts'): Promise<string> {
    ...
    // 1. Primary: Cloudflare R2 via NestJS Presigned URL (0 egress fees!)
    try {
      ...
      if (presignRes.data?.uploadUrl && presignRes.data?.publicUrl) {
        let binaryData: any;
        if (Platform.OS !== 'web' && typeof File === 'function') {
          try {
            const file = new File(uri);
            binaryData = await file.bytes();
          } catch {
        ...
        const r2Res = await fetch(presignRes.data.uploadUrl, {
          method: 'PUT',
          headers: {
            'Content-Type': media.mimeType || (media.type === 'video' ? 'video/mp4' : 'image/jpeg'),
          },
          body: binaryData,
        });
      ...
      console.warn('NestJS/R2 upload failed, using Supabase Storage fallback:', r2Err);
    }
