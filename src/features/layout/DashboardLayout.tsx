...
export default function DashboardLayout({ isLoggedIn }: DashboardLayoutProps) {
    ...
    const requestNotificationPermission = async () => {
        ...
        try {
            ...
            // Push subscription will be stored via NestJS when endpoint is ready
            console.log('[Push] Subscribed for user:', user.id);
        } catch (err) {
    ...
    useEffect(() => {
        ...
        const setupRealtime = async () => {
            // Use authUser from NestJS JWT context
            const user = authUser;
            if (cancelled) return;
            ...
            // Seed userProfile from NestJS auth user (Supabase is no longer available)
            const nestProfile: Profile = {
                id: user.id,
                name: user.name,
            ...
            // Use cached profile if available, else use NestJS data
            const cached = localStorage.getItem('ulink_profile_cache');
            if (cached) {
                try {
                    setUserProfile(JSON.parse(cached));
                } catch {
                    setUserProfile(nestProfile);
                }
            } else {
                setUserProfile(nestProfile);
                try { localStorage.setItem('ulink_profile_cache', JSON.stringify(nestProfile)); } catch {}
            }
        };
    ...
    useEffect(() => {
        ...
        const handleMessage = async (event: MessageEvent) => {
            ...
            const user = authUser;
            if (!user) return;
            // Push subscription renewal — will be wired to NestJS later
            console.log('[Push] Subscription renewed for user:', user.id);
        };
