...
if (supabaseUrl && supabaseKey) {
  try {
    ...
    console.warn('⚠️ Supabase client init skipped in sitemap generator');
  }
} else {
  console.log('ℹ️ No Supabase credentials found, generating static-only sitemap.');
}
  ...
  try {
    if (supabase) {
      try {
        ...
        console.warn('⚠️ Could not fetch dynamic sitemap items, using static routes:', queryErr.message || queryErr);
      }
    }
...
${urls.map(url => `  <url>
    ...
    console.warn('⚠️ Error generating sitemap, continuing build:', err.message || err);
  }
}
